...
func (p *Protocol) serializeBet(bet model.Bet) []byte {
	betBytes := make([]byte, 0)

	//put fixed fields
	betBytes = append(betBytes, p.getFieldBytesForUint32(uint32(bet.AgencyId))...)
	betBytes = append(betBytes, p.getFieldBytesForUint32(uint32(bet.Document))...)
	betBytes = append(betBytes, p.getFieldBytesForUint32(uint32(bet.Number))...)

	birthdateBytes := []byte(bet.Birthdate)
	betBytes = append(betBytes, birthdateBytes...)

	//put dinamic fields that have a length prefix
	betBytes = append(betBytes, p.getDynamicField(bet.FirstName)...)
	betBytes = append(betBytes, p.getDynamicField(bet.LastName)...)

	return betBytes
}

func (p *Protocol) getFieldBytesForUint32(field uint32) []byte {
	fieldBytes := make([]byte, FIXED_FIELDS_SIZE_BYTES)
	binary.BigEndian.PutUint32(fieldBytes, field)
	return fieldBytes
}

func (p *Protocol) getDynamicField(field string) []byte {
	//field in bytes
...
func (p *Protocol) deserializeBet(betBytes []byte) (model.Bet, error) {
	...
	agencyId, position := p.getInt32FromBytes(betBytes, position)
	document, position := p.getInt32FromBytes(betBytes, position)
	number, position := p.getInt32FromBytes(betBytes, position)

	birthdate := string(betBytes[position : position+BIRTHDATE_LENGTH])
	position += BIRTHDATE_LENGTH

	firstName, position, err := p.readDynamicField(betBytes, position)
	if err != nil {
		return model.Bet{}, err
	}

	lastName, _, err := p.readDynamicField(betBytes, position)
	if err != nil {
		return model.Bet{}, err
	}
...
func (p *Protocol) getInt32FromBytes(bytes []byte, position int) (int32, int) {
	return int32(binary.BigEndian.Uint32(bytes[position : position+FIXED_FIELDS_SIZE_BYTES])), position + FIXED_FIELDS_SIZE_BYTES
}

func (p *Protocol) readDynamicField(betBytes []byte, position int) (string, int, error) {
